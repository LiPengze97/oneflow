...
#include "oneflow/core/graph/collective_boxing_task_node.h" //可能要改
#include "oneflow/core/graph/slice_boxing_task_node.h"
#include "oneflow/core/graph/collective_boxing_pack_task_node.h" //可能要改
#include "oneflow/core/graph/collective_boxing_unpack_task_node.h" //可能要改
...
//在Broadcast以及Reduce当中用得到，确定root 的 ID
...
class OfCollectiveBoxingBroadcastSubTskGphBuilder final : public SubTskGphBuilder {
  ...
  Maybe<SubTskGphBuilderStatus> Build(
    ...
    if (in_parallel_desc.parallel_num() == 1 && out_parallel_desc.parallel_num() > 1
        ...
        && out_sbp_parallel.has_broadcast_parallel()) { //从这里开始里面的逻辑，外面的都不用管了先
      ...
      // 上面所有的代码都是为了确定 root_parallel_id，建议不改
      const std::string op_name = "System-Boxing-OfCollectiveBoxingBroadcast-" + NewUniqueId();
      FOR_RANGE(int64_t, i, 0, out_parallel_desc.parallel_num()) {//今天下午的重头戏，把这部分看懂。
        auto* collective_node = ctx->task_graph()->NewNode<CollectiveBoxingGenericTaskNode>();
        OcclInitCollectiveNode(collective_node, out_parallel_desc, i, op_name, lbi,
...
OfCollectiveBoxingSubTskGphBuilder::OfCollectiveBoxingSubTskGphBuilder() {
    // 这个的作用也待观察，是从一个resource.proto中来的
